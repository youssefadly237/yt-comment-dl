...
class YoutubeCommentDownloader:
    ...
    def get_comments_from_url(
        ...
        html = response.text
        json_str = self.regex_search(html, YT_CFG_RE, default="")
        if json_str:
            ytcfg = json.loads(json_str)
        else:
            return  # Unable to extract configuration
        if language:
            ytcfg["INNERTUBE_CONTEXT"]["client"]["hl"] = language

        initial_data_str = self.regex_search(html, YT_INITIAL_DATA_RE, default="")
        if initial_data_str:
        ...
        while continuations:
            ...
            for comment in reversed(
                ...
                try:
                    parsed_time = dateparser.parse(result["time"].split("(")[0].strip())
                    if parsed_time is not None:
                        result["time_parsed"] = parsed_time.timestamp()
