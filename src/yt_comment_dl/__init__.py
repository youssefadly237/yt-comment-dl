...
def main(argv=None):
    ...
    parser.add_argument(
        "--url", "-u", help="Youtube URL for which to download the comments"
    )
    parser.add_argument(
    ...
    try:
        ...
        if (not youtube_id and not youtube_url) or not output:
            parser.print_usage()
            raise ValueError(
                "you need to specify a Youtube ID/URL and an output filename"
            )
        ...
        if comment is None:
            print("No comments found, no file created.")
            return  # Exit early, no file created
